...
*/
  // promise version
  let promesa1= promisifiedReadFile('poem-two/stanza-01.txt').then(stanza1 => blue(stanza1))
  let promesa2= promisifiedReadFile('poem-two/stanza-02.txt').then(stanza2 => blue(stanza2))

    Promise.all([promesa1,promesa2]).then(()=>console.log('done'))
...
*/
  // promise version
  let arrayPromesa= filenames.map(file=>
    promisifiedReadFile(file).then(stanza=>blue(stanza)))

  Promise.all(arrayPromesa).then(()=>console.log('done'))
